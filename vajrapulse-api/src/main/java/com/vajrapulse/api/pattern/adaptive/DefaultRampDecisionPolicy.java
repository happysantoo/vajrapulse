 ...
 * 
 * <p>
 * This policy uses configurable thresholds for:
 * <ul>
 ...
 * Decision logic:
 * <ul>
 * <li><strong>Ramp Up:</strong> Error rate &lt; threshold AND backpressure &lt;
 * rampUpThreshold</li>
 * <li><strong>Ramp Down:</strong> Error rate &gt;= threshold OR backpressure
 * &gt;= rampDownThreshold</li>
 * <li><strong>Sustain:</strong> Stable intervals count &gt;= required</li>
 * <li><strong>Recover:</strong> Backpressure &lt; 0.3 OR (recent error rate
 * &lt; threshold AND backpressure &lt; 0.5)</li>
 * </ul>
 * 
 * @since 0.9.9
 */
public final class DefaultRampDecisionPolicy implements RampDecisionPolicy {
     ...
     * @param backpressureRampUpThreshold   backpressure threshold for ramping up
     *                                      (default: 0.3)
     * @param backpressureRampDownThreshold backpressure threshold for ramping down
     *                                      (default: 0.7)
     */
    public DefaultRampDecisionPolicy(
        ...
        if (backpressureRampUpThreshold < 0.0 || backpressureRampUpThreshold > 1.0) {
            throw new IllegalArgumentException(
                    "Backpressure ramp up threshold must be between 0.0 and 1.0, got: " + backpressureRampUpThreshold);
        }
        if (backpressureRampDownThreshold < 0.0 || backpressureRampDownThreshold > 1.0) {
            throw new IllegalArgumentException("Backpressure ramp down threshold must be between 0.0 and 1.0, got: "
                    + backpressureRampDownThreshold);
        }
    ...
    public DefaultRampDecisionPolicy(double errorThreshold) {
        this(errorThreshold, 0.3, 0.7);
    }

    @Override
    public boolean shouldRampUp(MetricsSnapshot metrics) {
        return metrics.recentFailureRate() < errorThreshold
    ...
    public boolean shouldSustain(int stableIntervalsCount, int requiredIntervals) {
        return stableIntervalsCount >= requiredIntervals;
    }

    @Override
    public boolean canRecoverFromMinimum(MetricsSnapshot metrics) {
        // Recovery conditions: backpressure low OR (error rate low AND backpressure
        // moderate)
