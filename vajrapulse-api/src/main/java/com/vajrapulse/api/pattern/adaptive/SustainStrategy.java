...
public final class SustainStrategy implements PhaseStrategy {
    ...
    public double handle(PhaseContext context, long elapsedMillis) {
        ...
        if (decisionPolicy.shouldRampDown(metrics)) {
            ...
            pattern.transitionPhaseInternal(current, AdaptivePhase.RAMP_DOWN, elapsedMillis, current.stableTps(), newTps);
            return newTps;
        }
        ...
        if (phaseStartTime >= 0) {
            ...
            if (phaseDuration >= config.sustainDuration().toMillis()) {
                ...
                pattern.transitionPhaseInternal(current, AdaptivePhase.RAMP_UP, elapsedMillis, current.stableTps(), newTps);
                return newTps;
            }
        }
